...
![Project Lifecycle](https://cipherops-blog.github.io/CipherOps/projectManagement/illustrations/projectLifecycle.png)
...
A Gantt chart is a bar chart representing a project schedule, showing the start and end dates of each task. It typically includes task durations, dependencies between tasks, milestones, and the critical path, which identifies the sequence of tasks that determines the shortest possible project duration. A Gantt chart is generally constructed from a work breakdown structure that decomposes the project into manageable tasks. Software tools commonly used to create Gantt charts include Microsoft Project and GanttProject.

### 3.2 Burndown chart

A burndown chart is a graphical representation of work remaining over time, used primarily in iterative project management. A sprint burndown chart tracks remaining work within a single iteration, while a release burndown chart tracks remaining work across multiple iterations toward a release date. Reading a burndown chart allows a team to assess whether it is on track to complete the work planned for a given period. One limitation of the burndown chart is that it does not reflect changes in project scope unless these are explicitly annotated on the chart.

### 3.3 Effort and timeline estimation

Several techniques are used to estimate effort and duration on IT projects, including expert judgment, analogous estimation based on comparable past projects, parametric estimation using statistical relationships, and three-point estimation based on optimistic, pessimistic, and most likely scenarios. Planning poker is an estimation technique used in agile teams in which team members independently propose estimates before reaching consensus. Project duration is calculated based on task dependencies and the availability of assigned resources. Sources of estimation uncertainty include incomplete requirements and the use of unproven technology.

By the end of this section, learners should be able to construct a Gantt chart for a defined set of project tasks, interpret a burndown chart to evaluate project progress, and apply at least one estimation technique to a project task list.

## 4. Risk Management and Team Facilitation

This section covers methods for identifying and managing project risks, along with facilitation techniques used to support project teams.

### 4.1 Risk identification and analysis

A project risk is an uncertain event that, if it occurs, has a positive or negative effect on project objectives. Risks are typically identified through brainstorming sessions, checklists derived from past projects, and interviews with subject matter experts. Once identified, risks are analyzed using qualitative methods, which assess probability and impact on a relative scale, or quantitative methods, which assign numerical estimates to potential outcomes. The results of this analysis are recorded in a risk register, which lists each identified risk along with its estimated probability, its potential impact, and the person responsible for monitoring it.

### 4.2 Mitigation plans

For risks with a negative effect on project objectives, four response strategies are commonly used: avoidance, which eliminates the risk by changing the project approach; mitigation, which reduces the probability or impact of the risk; transfer, which shifts responsibility for the risk to a third party; and acceptance, which involves taking no preventive action. Corresponding strategies exist for risks with a positive effect, including exploitation, enhancement, sharing, and acceptance. Contingency plans are developed for risks that are accepted, and contingency reserves are allocated to address them if they occur. Identified risks are monitored throughout the project lifecycle to track changes in their probability or impact.

### 4.3 Facilitation techniques

The project manager, or in agile contexts the Scrum Master, is generally responsible for facilitating team meetings. Common facilitation formats include daily stand-up meetings, retrospectives held at the end of an iteration, and structured workshops. Techniques used to structure group discussion include timeboxing, which limits the duration of a discussion, and round-robin input collection, which ensures that each participant contributes before the discussion continues. Conflict within project teams is typically addressed through structured negotiation or mediation led by the project manager or a designated facilitator.

By the end of this section, learners should be able to build a risk register for a given project scenario, select an appropriate risk response strategy for a specific risk, and apply a facilitation technique to structure a team meeting.

<span style="color:#B8A9C9">Further discussion of the different project management models may be found at:</span>
<div class="pdf-embed" data-src="https://cipherops-blog.github.io/CipherOps/courses/projectManagement/illustration/PMmodels.pdf"></div>
