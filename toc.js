...
const createSubcategoryNode = (subcategory) => {

  const subcategoryWrapper = document.createElement('div');

  subcategoryWrapper.className = 'toc-subcategory';

  subcategoryWrapper.setAttribute('aria-expanded', 'true');



  const subcategoryHeader = document.createElement('button');

  subcategoryHeader.type = 'button';

  subcategoryHeader.className = 'toc-subcategory-header';

  subcategoryHeader.textContent = subcategory.name;

  subcategoryHeader.addEventListener('click', () => {

    handleCategoryToggle(subcategoryWrapper);
  ...
  subcategoryWrapper.appendChild(articleList);
...
const createCategoryNode = (category) => {
  ...
  if (category.subcategories) {
    ...
    category.subcategories.forEach((subcategory) => {
      ...
      const subcategoryNode = createSubcategoryNode(subcategory);

      articleList.appendChild(subcategoryNode);
